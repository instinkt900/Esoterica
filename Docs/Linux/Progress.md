...
**One known correctness bug is outstanding**, defect 3: `MeshCluster` is laid out as 40 bytes in
SPIR-V where Direct3D and C++ both say 32, because DXC will not merge bitfields of different
signedness. Recorded in full below. The P4.3 bindless binding model
...
| 4 - Shader Pipeline | **in progress.** DXC builds from source with two patches; all 46 shader stages compile, validate and link, and `CompileShaders.sh` runs them. P4.6 and the clip-space Y decision remain |
...
**P4.5 is done**: `CompileShaders.sh` exists, with four modes, and `--hotreload` is deliberately
not the default. **Criterion 3 is met in full**, including `spirv-val`. What is left is **P4.6**,
the structural diff of the generated output against the Windows build, which needs a Windows
machine, plus the two remaining decisions below.
...
back end refusing to merge bitfields of different signedness, which leaves `MeshCluster` at 40
bytes where Direct3D and C++ both say 32. That is a live correctness bug on Linux, it will look
like corrupt geometry in Phase 5, and it is not fixed. See the entry below.
