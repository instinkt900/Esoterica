...
end, and **all 46 shader stages compile**. `./CompileShaders.sh` exits 0 and fills
`_AutoGenerated/Shaders/`; the generated C++ compiles and links. The P4.3 bindless binding model
...
| 4 - Shader Pipeline | **in progress.** DXC builds from source with two patches; all 46 shader stages compile, `CompileShaders.sh` runs them and the generated code links. P4.6 and the clip-space Y decision remain |
...
not the default. What is left is **P4.6**, the structural diff of the generated output against
the Windows build, which needs a Windows machine, plus the two remaining decisions below.
