...
> | P8.8 A warning-free build | **done**, 2026-09-04. Unplanned. Three warning tiers, `-Werror` on this fork's sources, and eight upstream defects to report |
>
...
`-Werror`. The question that started it was whether the wall of warnings was upstream's or the
port's, and it had never been measured.

#### The measurement, because a raw count is misleading

All 888 translation units, recompiled with `-fsyntax-only` using the flags from
`compile_commands.json`: **85,097 diagnostics at 17,264 unique sites.**

| Origin | Unique sites | Share |
|---|---|---|
...
**Diagnostics and sites are different numbers and only one of them means anything.**
`-Wdeprecated-builtins` was 25,560 diagnostics from **8** EASTL header sites, re-reported once per
translation unit that included the header. `04-BuildAndRun.md` described the whole problem as those
EASTL headers and "several hundred" warnings; both halves were wrong.

**And this was a choice, not an inheritance.** `Toolchain.py` passed `-Wall -Wextra` to everything
and deliberately translated none of upstream's MSVC suppression list. Upstream is clean on MSVC
*because that list exists*.

#### Six upstream defects, and they are the reason not to just pass `-w`

Most `-Wformat` hits are LP64 artefacts - `uint64_t` is `unsigned long` here and `unsigned long long`
on Windows, so `%llu` is right there and merely mismatched here. These are not that. **Each one is a
defect on both platforms**, and each needs an upstream report:

| Site | What is wrong |
|---|---|
| `AnimationRootMotion.cpp:164` | `result == Quaternion::Identity;` - `==` written for `=`. **`GetRotation()` on an empty track returns an uninitialised quaternion** |
| `FileSystem.h:88`, `:97` | The `Blob` overloads of `WriteBinaryFile` and `UpdateBinaryFile` **never return**. UB; the caller reads an indeterminate `bool`. **Fixed here** - see below |
| `DebugView_EntityWorld.cpp:155, 338, 350` | `ComponentID`, `EntityID` and `Radians` passed through printf varargs against `%u` and `%.2f` |
| `Animation_ToolsGraphNode_Events.cpp` ×5 | `ImGui::Text( "Prefer Highest Event %" )` - a trailing `%` with no specifier after it |
| `Animation_ToolsGraphNode_State.cpp` ×3 | The format string ignores an argument that is passed |
| `ResourceEditor_AnimationGraph.cpp:3075` | `&m_userContext != nullptr` is always true. A dead guard |
| `ImguiGizmo_Translate.cpp:548` | `m_previousPosWS = m_previousPosWS;`. Harmless, and a refactor leftover in the file P8.4 found a crash in |

Three more "expression result unused" statements, in `PlayerInputState.cpp:29`,
`Animation_RuntimeGraph_Instance.cpp:1702` and `BinarySerialization.h:394`, compute nothing.

**Only `FileSystem.h` is fixed here**, because it is undefined behaviour and because it was the sole
cause of `-Wreturn-type` inside this fork's own translation units, which `-Werror` could not have
tolerated. Two characters, escalated, and registered in [TouchedFiles.md](TouchedFiles.md). The rest
are upstream's (Conventions rule 3) and are now rows in [Blocked.md](Blocked.md)'s report list.

#### Three warning tiers, keyed off lists the generator already had

`SourceLists.Project` already separates the three populations - `linux_sources` from
`LinuxSources.txt`, `autogenerated_sources` from the glob, and everything else from a `.vcxproj` - so
no new registry was needed. `NinjaGen.py` emits three rule variants per project and configuration
and picks one per source file. About 600 extra rules against 38,000 edges, and every edge unchanged.

| Tier | Policy |
|---|---|
| This fork's sources | `-Wall -Wextra -Werror`, minus four documented suppressions |
| Upstream sources | `-Wall -Wextra`, minus 32 flags, each commented with its site count |
| Generated code | `-w` |

**`-isystem` instead of `-I` for third-party and `External/` include paths** is the part that is not
a suppression: clang simply does not report a header reached that way. It removes all 25,560
`-Wdeprecated-builtins`, the 264 external sites, and **670 of the 883 non-fork diagnostics that leak
into this fork's own translation units.** It also reorders the include search path - `-isystem` is
searched after every `-I` - which is why every configuration was rebuilt from scratch rather than
incrementally.

#### What `-Werror` cost, and what it caught that the sweep did not

**Four suppressions survive on this fork's own sources, and each is load-bearing:**

- **The two missing-field-initializer flags are the Vulkan idiom.**
  `VkFooCreateInfo x = { VK_STRUCTURE_TYPE_FOO };` is correct - the rest is zero-initialised, which
  is what Vulkan asks for - and `RHI_Vulkan.cpp` does it 106 times. **Designated initialisers do not
  help**: clang 21 moves the identical diagnostic to `-Wmissing-designated-field-initializers`, which
  `-Wextra` also enables. Measured, not assumed.
- **`-Wno-unused-parameter` and `-Wno-unused-function` are leaks from upstream headers.** 199 unused
  parameters arrive through `IDataFile.h`, `EditorTool.h`, `ReflectedType.h` and `String.h`, and
  `ImguiX.h:501` declares `static void HelpMarker` in a header, which is unused in every translation
  unit that does not call it. Clang has no per-header suppression that would not also cover this
  fork's own headers under `Code/`. **An unused parameter in new Linux code will not be caught**;
  `WarningSweep.py --audit-fork` is how to look for one deliberately.

**Eight real warnings in this fork's code were fixed**, six of them unused parameters in `_Linux`
platform stubs, plus a dead `backgroundColor` in `ImguiX_Linux.cpp` and a dead
`g_maxPathBufferLength` in `FileSystemPath_Linux.cpp`.

**Shipping found 30 more that Debug and Release could not, and the sweep could not either.**
`EE_ASSERT` compiles to nothing in Shipping, so every
`VkResult const result = vkFoo(); EE_ASSERT( result == VK_SUCCESS );` leaves `result` unused - 30
sites in `RHI_Vulkan.cpp`, now `[[maybe_unused]]`. **This is the argument for building all nine
configurations rather than the one that is usually built.** `-fsyntax-only` at `-O0` cannot see them,
and neither can Debug.

#### Two traps in the tooling, both of which produced a confident wrong answer

**The sweep reported "no warnings" over a build that did not compile.** With `-Werror` in the
command, a re-enabled warning is printed as `error:`, and the script only matched `warning:`. It
therefore said the tree was clean while `ImguiX.h`'s `HelpMarker` was failing four translation units.
`WarningSweep.py` now counts non-zero exits separately and says so, and `--audit-fork` strips
`-Werror` along with the suppressions it is there to lift.

**And `ninja ... | tail` hides a failed build.** The pipeline's exit status is `tail`'s, so a
`subcommand failed` scrolls past as a success. Read the exit code, not the last line.

- Files added: `Docs/Linux/Scripts/WarningSweep.py`.
- Upstream files edited: `Code/Base/FileSystem/FileSystem.h` (two `return`s), registered in
  [TouchedFiles.md](TouchedFiles.md).
- Acceptance criteria met: none named this; it is unplanned work. It does close the "several hundred
  EASTL warnings" claim in [04-BuildAndRun.md](04-BuildAndRun.md), which was wrong.
- For the next agent: **run `python3 Docs/Linux/Scripts/WarningSweep.py` after a merge.** A new
  upstream warning class will surface as a single new flag to add to
  `Toolchain.UPSTREAM_WARNING_SUPPRESSIONS`, with its count. If it fires in one of this fork's own
  files instead, the build will already have failed.
...
- For the next agent: **there is now a Windows desk in the rotation.** The Windows queue in
  [Blocked.md](Blocked.md) is five rows rather than eight, and each is a comparison, a second
  configuration, or one later edit to re-check - none is a "this might be silently broken" any more.
