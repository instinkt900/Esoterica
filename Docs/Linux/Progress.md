...
**Phase: 2 (in progress).** The Reflector builds, links and runs on Linux. It parses the entire
codebase with no compile errors. It does not yet emit generated code: a later semantic pass
fails on Windows-gated `ImGuiX` types. See the Phase 2 entry below for exactly where it stops.

| Phase | Status |
|---|---|
| 0 - Build System | **done** |
| 1 - Base Platform Layer | **done** (Tester itself still blocked on Phase 2) |
| 2 - Reflector | in progress (P2.1-P2.5 done; reflection runs but generates nothing yet) |
...
**Finishing Phase 2.** The Reflector runs and parses everything; three type-resolution errors
remain, all in `ImGuiX`. See "Where reflection stops" below.
...
Acceptance criteria met: **1** (builds and links) and **7** (LLVM pinned and recorded).
Not met: **2, 3, 4, 5, 6** (nothing generated yet), **8** (Windows build not run), **9** is done.
...
## Where reflection stops

Everything parses. What fails now is the Reflector's own type resolution, on three `ImGuiX`
types:

```
incomplete type 'EE::ImGuiX::Gizmo' named in nested name specifier
no type named 'FilterWidget' in namespace 'EE::ImGuiX'
no type named 'TextBuffer' in namespace 'EE::ImGuiX'
```

These look like the same shape as the `FileSystem::Watcher` problem already fixed: a type behind
a Windows-only guard, which breaks reflection for everything that references it, because the
Reflector concatenates every project header into **one** translation unit and parses that. Start
there.

Two warnings worth reading while fixing it, both probably consequences rather than causes:
`'PostLoad' marked 'override' but does not override any member functions`, and a
`Runtime data file declaration detected (EE::Physics::PhysicsMaterialLibrary)` complaint.
...
- The Windows build has **not** been run against any of this.
