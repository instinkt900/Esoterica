...
end, and **all 46 shader stages compile**. The Reflector's shader pass exits 0 and fills
...
| 4 - Shader Pipeline | **in progress.** DXC builds from source with two patches; all 46 shader stages compile and the generated code links. P4.5 and P4.6 remain |
...
What is left is **P4.5**, `CompileShaders.sh`, which does not exist yet, and **P4.6**, the
structural diff of the generated output against the Windows build. Both were gated on the shader
pass exiting 0, and it does.
