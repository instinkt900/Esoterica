...
| `Code/EngineTools/Entity/EntitySerializationTools.cpp` | `<eastl/sort.h>` to `<EASTL/sort.h>`. Case mismatch. **Was missing from this registry until 2026-09-03**, found by the P9.1 registry check rather than by anything failing - upstream touched the file in `47e6293` and it merged cleanly, so nothing complained. The gap is the finding, not the edit. | 3 | done |
