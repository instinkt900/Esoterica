...
| 6 | **The three glTF import fixes.** All three are platform-neutral edits to upstream files, so they reach Direct3D 12 as fixes rather than as port shims, and **none has been compiled by MSBuild**. Two are self-contained (a flipped assert, a name fallback) and a build is the whole check. The third rewrote the LOD-to-submesh mapping in the shared mesh compiler, so also **recompile `Boulder.mesh`, `Floor.mesh`, `MaterialBall.mesh` and `SkyDome.mesh` on Windows and confirm they still compile** - on Linux all four came out byte-identical before and after. Then import the Khronos Sponza sample and confirm the editor survives selecting it | `Formats/GLTF.cpp`, `RawFileInspector.cpp`, `ResourceCompiler_RenderMesh.cpp` | [UpstreamIssues.md](UpstreamIssues.md) items 2, 3 and 4; Progress.md 2026-09-04 glTF entry |
