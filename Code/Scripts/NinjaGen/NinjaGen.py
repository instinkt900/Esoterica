...
// .esh first. That does not work on Linux yet: the generated HLSL indexes ResourceDescriptorHeap,
// the Direct3D 12 bindless model, and DXC's SPIR-V back end crashes on it. Settling that is a
// Phase 5 renderer decision, not a porting detail.
//
...
def write_shader_registration_placeholder( repo_root ):
    ...
    return ( f'{SHADER_REGISTRATION_DIRECTORY}/ShaderRegistration.h is a placeholder with no '
             'shaders in it. The Reflector\'s shader pass does not run on Linux yet, because '
             'DXC crashes compiling the bindless HLSL. Phase 5 settles that; until then a Linux '
