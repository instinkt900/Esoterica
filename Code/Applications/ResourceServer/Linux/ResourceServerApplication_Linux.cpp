...
namespace EE
{
    ...
    {
        // The Win32 sibling asks for confirmation when clients are still connected. It cannot
        // here: MessageDialog::Confirmation on Linux logs and returns false until P7.2 makes it
        // real, so asking would refuse every exit and trap the user in a window that will not
        // close. Warn and exit instead.
        int32_t const numConnectedClients = m_resourceServer.GetNumConnectedClients();
        if ( numConnectedClients > 0 )
        {
            EE_LOG_WARNING( LogCategory::Resource, "Resource Server", "Exiting with %d client(s) still connected", numConnectedClients );
        }
